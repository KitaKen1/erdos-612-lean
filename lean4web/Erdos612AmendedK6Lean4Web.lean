...
/-!
# Erdős Problem 612 — amended K₆ counterexample

Standalone Lean4Web file for Lean v4.34.0-rc2.
The main result is `Erdos612.amended_k5_refuted : ¬ AmendedConjectureAt 5`.
All graph-theoretic and arithmetic proof components are included.
-/
...
/-!
# Final target definition: the same conjecture as FClike

The two definitions below are copied verbatim from the FClikeLean draft.
They avoid the FormalConjecturesUtil `answer` notation and category attributes.
The conjecture itself is proved only in the final target file.

Here `k = 5` corresponds to the forbidden clique K₆; the target is
`¬ AmendedConjectureAt 5`, not `AmendedConjectureAt 6` (the K₇ case).
-/

namespace Erdos612

/-- A diameter upper bound with one additive constant shared by all graphs. -/
...
This file contains no proof holes.
All finite table checks use `decide`, not `native_decide`.
-/
...
/-- One row of the PDF table, (qᵢ, aᵢ, bᵢ). -/
...
/-- Ordinary classes have 22aᵢ vertices; the final class has max(1, 22bᵢ) vertices. -/
...
/-- For adjacent layers with six classes in total, delete the final-class pair. -/
...
/-- Number of neighbors of class a in the three-layer periodic window. -/
...
/-- The two left and right cap layers, each containing 2200 vertices. -/
abbrev CapVertex := Fin 2 × Fin 2200

/-- Two left cap layers, m periodic blocks, and two right cap layers. -/
...
This part is independent of the graph-theoretic proof.
The minimum degree is fixed at 2200 while the number of periods m grows.
-/

namespace Erdos612AmendedK6

/-- Algebraic calculation of the right-hand side of equation (14) in the PDF. -/
...
/-- An explicit period count whose error exceeds any prescribed natural N. -/
...
/-!
# Definition of the actual graph

The definition is the edge rule from the PDF, not an empty placeholder.
The construction includes proofs of connectedness, K₆-freeness, minimum degree,
and the diameter lower bound.
-/
...
/-! ## Reindexing vertices as `SimpleGraph (Fin n)` for FClike -/
...
This file contains no `sorry`. It uses the constructed graph to refute
the amended conjecture at k = 5, with no additional local assumptions.
-/
...
/-- FC-style presentation corresponding to `answer(False) ↔ AmendedConjectureAt 5`.
This file avoids the FC-specific `answer` notation. -/
theorem erdos_612.variants.amended_k5 :
    False ↔ Erdos612.AmendedConjectureAt 5 := by
  constructor
  · intro h
    exact h.elim
