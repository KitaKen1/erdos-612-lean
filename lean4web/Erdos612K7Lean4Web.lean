...
/-!
# Erdős Problem 612 — standalone K₇ counterexample for Lean4Web

Target environment: Lean / Mathlib v4.34.0-rc2.
This file is self-contained apart from Mathlib.
The main theorems refute both the original `r = 3` target and the amended
`k = 6` target.
-/
...
/-- The Formal-Conjectures-shaped diameter bound with a divisibility condition. -/
...
/-- FC-style presentation of the original `r = 3` target. -/
theorem erdos_612.variants.original_k7 :
    False ↔ Erdos612.OriginalOddConjectureAt 3 := by
  constructor
  · intro h
    exact h.elim
  · exact Erdos612.original_k7_refuted

/-- FC-style presentation of the amended `k = 6` target. -/
theorem erdos_612.variants.amended_k6 :
    False ↔ Erdos612.AmendedConjectureAt 6 := by
  constructor
  · intro h
    exact h.elim
